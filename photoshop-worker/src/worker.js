...
async function processFolder(jobFolder,outbox){
  ...
  try{
    ...
    console.error(e); await writeJson(out,'result.json',{status:'failed',jobId:job.id,error:e.message||String(e),finishedAt:new Date().toISOString()}); log(`✗ ${job.id}: ${e.message}`); setState(`失败：${e.message}`,'bad'); return true;
  }
}

async function scanOnce(){
  if(state.busy)return; if(!state.template||!state.workspace){setState('请先选择母版和 Workspace','bad');return;}
  state.busy=true;
  try{
  ...
  }catch(e){console.error(e);log(`扫描失败：${e.message}`);setState(`扫描失败：${e.message}`,'bad');}
  finally{state.busy=false;}
}
function start(){if(state.timer)clearInterval(state.timer);state.running=true;$('#toggle').textContent='停止自动接单';state.timer=setInterval(scanOnce,2000);scanOnce();}
function stop(){state.running=false;if(state.timer){clearInterval(state.timer);state.timer=null;}$('#toggle').textContent='启动自动接单';setState('自动接单已停止');}

$('#pickTemplate').addEventListener('click',async()=>{const e=await fs.getFileForOpening({types:['psd'],allowMultiple:false});if(e){state.template=e;refreshPaths();await savePrefs();log(`母版：${display(e)}`);}});
$('#pickWorkspace').addEventListener('click',async()=>{const e=await fs.getFolder();if(e){state.workspace=e;refreshPaths();await ensureFolder(e,'inbox');await ensureFolder(e,'outbox');await savePrefs();log(`Workspace：${display(e)}`);}});
