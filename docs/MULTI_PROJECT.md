...
三份 PSD 均由 `tools/psd-template/generate.psjs` 生成，因此共享：
...
项目注册集中在 `web/project/project-registry.js`。三个项目共享同一套会议字段，但分别绑定自己的 `textLayoutProfile` 与 `assetLayoutProfile`。
...
如果新增项目仍由同一 `tools/psd-template/generate.psjs` 生成、只更换底板：

1. 在 `web/project/project-registry.js` 增加项目。
2. 在 `web/project/text-layout-profiles.js` 增加独立文字 profile。
3. 在 `web/project/asset-layout-profiles.js` 增加独立素材 profile。
