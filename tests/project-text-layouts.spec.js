...
test('active project installs its profile and preview overrides stay project-local', async ({ page }) => {
  ...
  const yiluOverride = await page.evaluate(() => localStorage.getItem('posterPreviewLayout:chronic-care-2026'));
  expect(yiluOverride).toContain('section-chair');

  await page.goto('/?project=tonghu-jiankang');
  await expect(page.locator('#posterCanvas')).toBeVisible();
  const tonghu = await page.evaluate(() => ({
    profile: window.POSTER_PROJECT.textLayoutProfile,
    localOverride: localStorage.getItem('posterPreviewLayout:tonghu-jiankang'),
    chairX: window.POSTER_PROJECT.textItems.find(item => item.id === 'section-chair')?.x,
  }));
  expect(tonghu.profile).toBe('tonghu-jiankang-text-v1');
  expect(tonghu.localOverride).toBeNull();
