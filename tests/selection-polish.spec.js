...
async function installMock(page) {
  ...
  await page.addInitScript(({ projectRef, userId, accessToken }) => {
    localStorage.setItem(`sb-${projectRef}-auth-token`, JSON.stringify({
      ...
      token_type: 'bearer',
      user: { id: userId, aud: 'authenticated', role: 'authenticated', app_metadata: { provider: 'anonymous', providers: ['anonymous'] }, user_metadata: {}, identities: [], is_anonymous: true, created_at: new Date().toISOString() },
    }));
  ...
  await page.route(`https://${PROJECT_REF}.supabase.co/**`, async route => {
    ...
    if (url.pathname === '/rest/v1/poster_service_status') {
      return route.fulfill({ status: 200, headers, body: JSON.stringify({ agent_last_seen_at: new Date().toISOString(), worker_last_seen_at: new Date().toISOString(), worker_status: 'ready' }) });
    }
    ...
    if (url.pathname === '/rest/v1/poster_jobs') return route.fulfill({ status: 200, headers: { ...headers, 'content-range': '0-0/0' }, body: '[]' });
...
async function ready(page) {
  await page.goto('/');
  await page.waitForFunction(() => Boolean(window.posterTextLayout && window.posterAssetLayout && window.posterLayoutTool && window.posterSelectionPolish && window.Moveable));
...
test('turning off the unified layout tool clears the active asset selection', async ({ page }) => {
  ...
  await page.locator('[data-layout-mode]').click();
  await expect.poll(() => page.evaluate(() => ({ layout: window.posterLayoutTool.isEnabled(), asset: window.posterAssetLayout.isEnabled(), selected: window.posterAssetLayout.getSelectedKeys() }))).toEqual({ layout: false, asset: false, selected: [] });
});
...
test('one layout tool switches context by the selected object and clears stale selection', async ({ page }) => {
  await enableLayout(page);
  await page.locator('[data-preview-text-id="section-chair"]').click();
  await expect.poll(() => page.evaluate(() => ({ text: window.posterTextLayout.isEnabled(), asset: window.posterAssetLayout.isEnabled(), textCount: window.posterTextLayout.getSelectedIds().length }))).toEqual({ text: true, asset: false, textCount: 1 });
  await page.locator('.canvas-asset-slot[data-key="qr"]').click();
  await expect.poll(() => page.evaluate(() => ({ context: window.posterLayoutTool.getContext(), text: window.posterTextLayout.isEnabled(), asset: window.posterAssetLayout.isEnabled(), textCount: window.posterTextLayout.getSelectedIds().length, assetKeys: window.posterAssetLayout.getSelectedKeys() }))).toEqual({ context: 'asset', text: false, asset: true, textCount: 0, assetKeys: ['qr'] });
  await page.locator('[data-preview-text-id="section-chair"]').click();
  await expect.poll(() => page.evaluate(() => ({ context: window.posterLayoutTool.getContext(), text: window.posterTextLayout.isEnabled(), asset: window.posterAssetLayout.isEnabled(), assetCount: window.posterAssetLayout.getSelectedKeys().length, textIds: window.posterTextLayout.getSelectedIds() }))).toEqual({ context: 'text', text: true, asset: false, assetCount: 0, textIds: ['section-chair'] });
});