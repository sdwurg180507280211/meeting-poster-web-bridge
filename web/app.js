(() => {
  ...
  const peopleDef = [
    ['chair','会议主席'], ['speaker1','讲者一'], ['speaker2','讲者二']
  ];
  const peopleState = {};
  const scheduleState = Array.from({length:4}, () => ({time:'',content:'',speaker:'',chair:''}));
  let client = null;
  let user = null;
  let activeJob = null;
  let pollTimer = null;
  ...
  function safeExt(name) { const m = String(name||'').toLowerCase().match(/\.([a-z0-9]+)$/); return m ? m[1].replace('jpeg','jpg') : 'png'; }
  ...
  function buildPeople() {
    ...
    peopleDef.forEach(([key,label]) => {
      ...
      ['zoom','x','y'].forEach(axis => document.getElementById(`${key}-${axis}`).addEventListener('input', e => {
        const v=Number(e.target.value); document.getElementById(`${key}-${axis}-v`).textContent = axis==='zoom'?`${v}%`:String(v);
  ...
  function onAvatarFile(key,file){
    if(!file) return;
    const st=peopleState[key]; st.file=file;
    if(st.url) URL.revokeObjectURL(st.url);
    st.url=URL.createObjectURL(file);
    const img=document.getElementById(`${key}-img`); img.src=st.url; img.onload=()=>renderAvatar(key);
  }

  function renderAvatar(key){
    const st=peopleState[key]; const img=document.getElementById(`${key}-img`); if(!st.file || !img.naturalWidth) return;
    const box=110; const cover=Math.max(box/img.naturalWidth,box/img.naturalHeight)*st.crop.zoom;
  ...
  function buildSchedule(){
    const root=document.getElementById('schedule'); root.innerHTML='';
    for(let i=0;i<4;i++){
  ...
  document.getElementById('qrFile').addEventListener('change', e => {
    const f=e.target.files[0]; const img=document.getElementById('qrPreview');
    if(!f){img.style.display='none';return;} img.src=URL.createObjectURL(f); img.style.display='block';
  });
  ...
  async function init(){
    if(!cfg.SUPABASE_URL || cfg.SUPABASE_URL.includes('YOUR_PROJECT') || !cfg.SUPABASE_PUBLISHABLE_KEY || cfg.SUPABASE_PUBLISHABLE_KEY.includes('YOUR_')){
      cloudState.textContent='请先配置 config.js'; cloudState.className='badge bad'; return;
    }
    ...
    user=session.user; cloudState.textContent='云端已连接'; cloudState.className='badge'; log(`anonymous user ${user.id}`);
  }
  ...
  async function uploadFile(path,file){
    const {error}=await client.storage.from(cfg.BUCKET||'poster-assets').upload(path,file,{contentType:file.type||'application/octet-stream',upsert:false});
    if(error) throw error;
  }

  document.getElementById('posterForm').addEventListener('submit', async e => {
    e.preventDefault(); if(!client||!user){alert('Supabase 尚未连接');return;}
    try{
      submitBtn.disabled=true; downloads.innerHTML=''; resultPreview.style.display='none';
      ...
      const active=meeting.schedule.filter(r=>r.time||r.content||r.speaker||r.chair); if(!active.length) throw new Error('请至少填写一行日程');
      active.forEach((r,i)=>{if(!r.time||!r.content) throw new Error(`已填写的日程必须包含时间和内容`);});
      const jobId=uuid(); const base=`${user.id}/${jobId}/input`;
      setStatus('正在上传素材…','uploading');
      const assets={};
      for(const [key] of peopleDef){
        const st=peopleState[key], path=`${base}/${key}.${safeExt(st.file.name)}`;
        await uploadFile(path,st.file); assets[key]={storagePath:path,crop:st.crop,originalName:st.file.name};
      }
      const qrPath=`${base}/qr.${safeExt(qr.name)}`; await uploadFile(qrPath,qr); assets.qrCode={storagePath:qrPath,originalName:qr.name};
      const payload={meeting,assets};
      const {error}=await client.from('poster_jobs').insert({id:jobId,owner_id:user.id,status:'pending',payload}); if(error) throw error;
      activeJob=jobId; setStatus('任务已提交，等待你的 Mac Photoshop 接单…','pending'); log(`job ${jobId} created`); startPolling();
    }catch(err){console.error(err);setStatus(`提交失败：${err.message}`,'failed');submitBtn.disabled=false;}
  });

  function setStatus(text,status){
    jobStatus.textContent=text; jobStatus.className=`status-box ${status==='succeeded'?'ok':status==='failed'?'bad':''}`;
    const map={pending:'① 已提交 → 等待 Mac',claimed:'② Mac Agent 已接单',rendering:'③ Photoshop 正在生成',uploading:'④ 正在上传结果',succeeded:'⑤ 已完成',failed:'生成失败'};
    progressSteps.textContent=map[status]||'';
  }

  function startPolling(){ if(pollTimer) clearInterval(pollTimer); pollTimer=setInterval(pollJob,2000); pollJob(); }
  async function pollJob(){
    ...
    const label={pending:'等待 Mac Photoshop 接单…',claimed:'Mac Agent 已接单，正在准备素材…',rendering:'Photoshop 正在生成正式海报…',uploading:'Photoshop 已完成，正在上传结果…',succeeded:'生成完成',failed:`生成失败：${data.error_message||'未知错误'}`};
    setStatus(label[data.status]||data.status,data.status);
    if(data.status==='succeeded'){
      clearInterval(pollTimer); pollTimer=null; submitBtn.disabled=false;
  ...
  async function showResults(job){
    ...
    const [png,psd]=await Promise.all([
      bucket.createSignedUrl(job.result_png_path,1800), bucket.createSignedUrl(job.result_psd_path,1800)
    ]);
