(() => {
  ...
  function installMobileTextKeyboardGuard() {
    ...
    const sync = (visibleViewport = visualViewport) => {
      const sheet = document.querySelector('.mobile-text-sheet:not([hidden])');
      if (!sheet || !mobileQuery.matches) return;
      const top = Math.max(0, Number(visibleViewport?.offsetTop || 0));
      const height = Math.max(1, Number(visibleViewport?.height || window.innerHeight || 1));
